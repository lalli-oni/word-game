...
{#if errorService.error}
    <div class="fixed inset-0 bg-red-950 z-1000 overflow-y-auto p-8 font-mono text-white flex flex-col items-center">
        <div class="w-full max-w-2xl bg-black/40 border-4 border-red-500 rounded-4xl p-8 shadow-2xl">
            <h1 class="text-4xl font-black text-red-500 mb-4 italic uppercase tracking-tighter">FATAL ERROR</h1>
            <p class="text-xl font-bold mb-8 text-red-200">The journey cannot continue due to a system failure.</p>
            <div class="space-y-6">
                <div><h2 class="text-xs font-black text-red-500 uppercase tracking-widest mb-2">Message</h2><p class="bg-red-900/30 p-4 rounded-xl border border-red-500/30">{errorService.error.message}</p></div>
                {#if errorService.error.stack}<div><h2 class="text-xs font-black text-red-500 uppercase tracking-widest mb-2">Stack Trace</h2><pre class="bg-black/60 p-4 rounded-xl border border-red-500/20 text-[10px] overflow-x-auto">{errorService.error.stack}</pre></div>{/if}
                <div><h2 class="text-xs font-black text-red-500 uppercase tracking-widest mb-2">Environment Context</h2><div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px]">{#each Object.entries(errorService.error.context || {}) as [k, v]}<div class="bg-white/5 p-2 rounded flex justify-between border border-white/5"><span class="opacity-50 uppercase">{k}</span><span class="font-bold truncate ml-4" title={v}>{v}</span></div>{/each}</div></div>
            </div>
            <div class="mt-12 flex gap-4"><Button variant="danger" onclick={() => location.reload()}>RELOAD PAGE</Button><Button variant="secondary" onclick={() => { localStorage.clear(); location.reload(); }}>CLEAR ALL DATA & RESET</Button></div>
        </div>
    </div>
{/if}
...
<div class="fixed inset-0 bg-slate-900 text-white flex flex-col items-center overflow-hidden overscroll-none h-[100dvh]">
  ...
  <div class="flex-1 w-full max-w-lg px-4 flex flex-col min-h-0 relative scrollbar-gutter-stable">
    ...
    <div class="flex-1 flex flex-col min-h-0 relative py-2">
        {#if showTopIndicator}<div class="absolute top-0 left-0 right-0 h-12 bg-linear-to-b from-slate-900 to-transparent z-10 pointer-events-none"></div>{/if}
        {#if showBottomIndicator}<div class="absolute bottom-0 left-0 right-0 h-12 bg-linear-to-t from-slate-900 to-transparent z-10 pointer-events-none"></div>{/if}
        ...
        <div bind:this={scrollContainer} onscroll={handleScroll} class="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2 pb-2 scroll-smooth">
            {#each game.history.slice(1) as move, i}
                {#if i < game.history.length - 2 || !game.isGameOver}
                    <WordRow type="history" historyIndex={i} move={move}>
            ...
            {#if !game.isGameOver}
                <div class="flex flex-col gap-2 shrink-0">
                    <WordRow type="input" historyIndex={game.history.length}>
    ...
    <div class="flex-none pt-0 pb-8">
        ...
        <div class={game.isGameOver ? 'cursor-pointer group/goal active:scale-[0.98] transition-all' : ''} onclick={() => game.isGameOver && successDialog.showModal()}>
            <WordRow type="destination" gameOver={game.isGameOver}>
                {#snippet card()}
                  <div class="relative">
                      {#if game.isGameOver}
